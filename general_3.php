        ...
        <div class="tm_invoice_in">
          <div class="tm_invoice_head tm_top_head tm_mb15 tm_align_center">
            <div class="tm_invoice_left">
              <div class="tm_logo"><img src="assets/img/logo2.png" alt="Logo"></div>
            </div>
          ...
          <div class="tm_invoice_info tm_mb25">
            <div class="tm_card_note tm_mobile_hide"><b class="tm_primary_color">Payment Method: </b><span
                id="paymentMethod"></span></div>
            <div class="tm_invoice_info_list tm_white_color">
              <p class="tm_invoice_number tm_m0">Invoice No: <b id="invoiceNumber"></b></p>
              <p class="tm_invoice_date tm_m0">Date: <b id="invoiceDate"></b></p>
            </div>
                ...
                <table class="tm_mb15">
                  <tbody>
                    <tr class="tm_gray_bg">
                      ...
                      <td class="tm_width_3 tm_primary_color tm_bold tm_text_right" id="subtotal"></td>
                    </tr>
                    <tr class="tm_gray_bg">
                      <td class="tm_width_3 tm_primary_color">Tax <span class="tm_ternary_color" id="taxValue"></span></td>
                      <td class="tm_width_3 tm_primary_color tm_text_right" id="tax"></td>
                    </tr>
                    <tr class="tm_accent_bg">
          ...
          <div class="tm_note tm_text_center tm_font_style_normal">
            ...
            <p class="tm_m0">All claims relating to quantity or shipping errors shall be waived by Buyer unless made in
              writing to <br>Seller within thirty (30) days after delivery of goods to the address stated.</p>
          </div><!-- .tm_note -->
        </div>
      </div>
      ...
      fetch('assets/invoices.json?v=' + new Date().getTime())
        ...
        .then(data => {
          ...
          document.getElementById("invoiceTo").innerHTML = `
            ${invoice.invoice_to.name} <br>
            ${invoice.invoice_to.address} <br>
            ${invoice.invoice_to.country} <br>
          ...
          invoice.items.forEach((item, index) => {
            ...
            row.innerHTML = `
              <td class="tm_width_3">${index + 1}. ${item.Item}</td>
              <td class="tm_width_4">${item.Description}</td>
              <td class="tm_width_2 tm_text_right">${invoice.currencyFormat}${item.total_price.toFixed(2)}</td>
            `;
            invoiceItems.appendChild(row);
          });

          const subtotal = invoice.items.reduce((sum, item) => sum + item.total_price, 0);
          const tax = (subtotal / 100) * invoice.tax;
          const grandTotal = subtotal + tax;
          document.getElementById("taxValue").textContent = `(${invoice.tax}%)`;
          document.getElementById("subtotal").textContent = `${invoice.currencyFormat}${subtotal.toFixed(2)}`;
          document.getElementById("tax").textContent = `+${invoice.currencyFormat}${tax.toFixed(2)}`;
          document.getElementById("grandTotal").textContent = `${invoice.currencyFormat}${grandTotal.toFixed(2)}`;
